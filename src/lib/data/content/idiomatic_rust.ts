...
Using \`match\` works well, but it can be verbose. \`Result<T, E>\` has helper methods that do the same thing.

### unwrap
If the \`Result\` is \`Ok\`, \`unwrap\` returns the value inside.
If it is \`Err\`, \`unwrap\` calls \`panic!\`.
...
### expect
Similarly to \`unwrap\`, it returns the value or panics. However, \`expect\` lets you choose the panic error message.
**This is generally preferred** because it gives more context.
